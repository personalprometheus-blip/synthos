...
def register_cron() -> bool:
    ...
    new_entries = "\n".join([
        ...
        "",
        "# Boot services (all times America/New_York)",
        f"@reboot sleep 60 && {py} {boot_script} >> {boot_log} 2>&1",
        f"@reboot sleep 90 && {py} {watchdog} &",
        f"@reboot sleep 90 && {py} {portal} &",
        "",
        "# RETAIL SESSIONS — via scheduler (handles multi-tenant customer routing)",
        "# Open session — 9:30am ET weekdays",
        f"30 9 * * 1-5    {py} {scheduler} --session open  >> {logf('scheduler')} 2>&1",
        f"31 9 * * 1-5    {py} {heartbeat} --session open  >> {logf('heartbeat')} 2>&1",
        "# Midday session — 12:30pm ET weekdays",
        f"30 12 * * 1-5   {py} {scheduler} --session midday >> {logf('scheduler')} 2>&1",
        f"31 12 * * 1-5   {py} {heartbeat} --session midday >> {logf('heartbeat')} 2>&1",
        "# Close session — 3:30pm ET weekdays",
        f"30 15 * * 1-5   {py} {scheduler} --session close  >> {logf('scheduler')} 2>&1",
        f"31 15 * * 1-5   {py} {heartbeat} --session close  >> {logf('heartbeat')} 2>&1",
        "# News — hourly during market hours + overnight runs",
        f"5 9-15 * * 1-5  {py} {scheduler} --session news      >> {logf('scheduler')} 2>&1",

...
def register_cron() -> bool:
    ...
    new_entries = "\n".join([
        ...
        f"@reboot sleep 60 && {sys.executable} {boot_script} >> {boot_log} 2>&1",
        f"@reboot sleep 90 && {sys.executable} {watchdog} &",
        f"@reboot sleep 90 && {sys.executable} {portal} &",
        f"55 3 * * 6  {sys.executable} {shutdown_scr}",
        f"0  4 * * 6  sudo reboot",
        "",
    ])

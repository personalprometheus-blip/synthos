...
// ── INTRO STATE — one-shot: laser scan reveals SYNTHOS, morphs into waves ──
// P1: scan descends top→bottom, SYNTHOS text reveals behind it
// P2: scan settles at bottom, primary wave amplitude ramps 0→full
// P3: SYNTHOS text fades, additional wave layers fade in → hand off to normal wave
var _apIntroActive = false;
var _apIntroStart  = null;
var _apIntroP1 = 1100;   // scan descent (ms)
var _apIntroP2 = 600;    // settle / flat-to-curve morph
var _apIntroP3 = 500;    // text fade + layer build-up
var _apIntroTotal = _apIntroP1 + _apIntroP2 + _apIntroP3;
(function _apInitIntro() {
  try {
    ...
    _apIntroActive = true;
    // Set flag immediately so a refresh mid-animation doesn't replay
    sessionStorage.setItem('synthos_intro_played', '1');
  } catch (e) {}
})();

function _apDrawSynthosText(ctx, w, h, alpha) {
  ...
  ctx.shadowBlur = 22;
  ctx.shadowColor = 'rgba(0,245,212,' + (alpha * 0.85) + ')';
  ctx.fillStyle = 'rgba(0,245,212,' + (alpha * 0.88) + ')';
  ctx.fillText('SYNTHOS', w / 2, h / 2 + 1);
  ctx.shadowBlur = 0;
  ctx.lineWidth = 1.2;
  ctx.strokeStyle = 'rgba(220,250,255,' + (alpha * 0.9) + ')';
  ctx.strokeText('SYNTHOS', w / 2, h / 2 + 1);
  ctx.restore();
}

function _apDrawScanLine(ctx, w, y, alpha) {
  // Soft glow trail above the scan line
  var trailH = Math.min(22, y + 4);
  if (trailH > 2) {
    var grad = ctx.createLinearGradient(0, y - trailH, 0, y);
    grad.addColorStop(0, 'rgba(0,245,212,0)');
    grad.addColorStop(1, 'rgba(0,245,212,' + (alpha * 0.4) + ')');
    ctx.fillStyle = grad;
    ctx.fillRect(0, y - trailH, w, trailH);
  }
  // Bright scan line
  ctx.save();
  ...
  ctx.lineWidth = 1.6;
  ctx.shadowBlur = 12;
  ctx.shadowColor = 'rgba(0,245,212,' + (alpha * 0.95) + ')';
  ctx.beginPath();
  ctx.moveTo(0, y);
  ctx.lineTo(w, y);
  ctx.stroke();
...
function _apDrawIntroWave(ctx, w, h, ampScale, layerCount, alphaMult) {
  // Stripped-down version of the main wave loop for the morph transition.
  // amplitudeScale ramps 0→1; layerCount ramps up; alphaMult fades layers in.
  var midY = h / 2;
...
function _apDrawIntroFrame(ctx, w, h, elapsed) {
  if (elapsed < _apIntroP1) {
    // Phase 1: scan descent, SYNTHOS reveal
    var p = elapsed / _apIntroP1;
    // ease-out — slightly slows at bottom
    var pe = 1 - Math.pow(1 - p, 2);
    var scanY = pe * h;
    ...
    ctx.clip();
    _apDrawSynthosText(ctx, w, h, 1);
    ctx.restore();

    _apDrawScanLine(ctx, w, scanY, 1);
  } else if (elapsed < _apIntroP1 + _apIntroP2) {
    // Phase 2: text fully visible, scan line holds + fades, wave starts curving
    var p = (elapsed - _apIntroP1) / _apIntroP2;
    var ampScale = p;   // 0→1
    _apDrawIntroWave(ctx, w, h, ampScale, 2, 0.5 + p * 0.5);
    _apDrawSynthosText(ctx, w, h, 1);
    _apDrawScanLine(ctx, w, h - 1.5, 1 - p);
  } else {
    // Phase 3: SYNTHOS fades, wave layers build to full
    var p = (elapsed - _apIntroP1 - _apIntroP2) / _apIntroP3;
    var layers = 2 + Math.floor(p * 4);
    _apDrawIntroWave(ctx, w, h, 1, layers, 1);
    _apDrawSynthosText(ctx, w, h, 1 - p);
  }
}
...
function _apDrawWave() {
  ...
  // Intro: laser-scan reveal + morph. Plays once per tab session.
  if (_apIntroActive) {
